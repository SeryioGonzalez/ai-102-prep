...
echo "Creating the data source $az_search_data_source_name"
...
echo "Creating the skillset $az_search_skillset_name"
jq --arg az_storage_account_connection_string $AZ_STORAGE_ACCOUNT_CONNECTION_STRING '.knowledgeStore.storageConnectionString=$az_storage_account_connection_string' skillset.json > temp.json && mv temp.json skillset.json
...
sed -i -e "s/\"storageContainer\": \".*-knowledge\"/\"storageContainer\": \"$context-knowledge\"/"                                                                  skillset.json             
sed -i -e "s/\"storageContainer\": \".*-images\"/\"storageContainer\": \"$context-images\"/"                                                                        skillset.json
...
echo "Creating the index $az_search_index_name"
...
echo "Creating the indexer $az_search_indexer_name" 
